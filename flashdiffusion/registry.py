...
Allows registering and discovering models, schedulers, pipelines, losses,
datasets, and other components by name — so users can swap them via config
without modifying source code.

Usage:
    from flashdiffusion.registry import MODELS, SCHEDULERS, PIPELINES

    @SCHEDULERS.register("MyScheduler")
    class MyScheduler:
        ...

    # Later, build from config
    scheduler = SCHEDULERS.build("MyScheduler", **kwargs)
"""
...
CONTROLNETS = Registry("controlnets")
ADAPTERS = Registry("adapters")
VIDEO = Registry("video")
UPSCALERS = Registry("upscalers")
