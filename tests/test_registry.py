...
def test_global_registries_exist():
    from flashdiffusion.registry import MODELS, SCHEDULERS, PIPELINES, LOSSES, DATASETS, CONTROLNETS

    assert MODELS.name == "models"
    assert SCHEDULERS.name == "schedulers"
    assert PIPELINES.name == "pipelines"
    assert LOSSES.name == "losses"
    assert DATASETS.name == "datasets"
    assert CONTROLNETS.name == "controlnets"
